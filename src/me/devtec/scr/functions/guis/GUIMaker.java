...
public class GUIMaker {
	...
	public void createCommand() {
		...
		cmd = CommandStructure.create(CommandSender.class, ScrCommand.PERMS_CHECKER, (s, structure, args) -> {
			if (!(s instanceof Player)) {
				s.sendMessage(ColorUtils.colorize("/cmd <player>"));
				return;
			}
			if (config.getBoolean("enabled"))
				open((Player) s);
		}).permission(permission()).selector(Selector.PLAYER, (s, structure, args) -> {
	...
	public void open(Player p) {
		...
		if (gui != null && !dynamic) {
			...
			if (config.exists("update"))
				updateAfter = TimeUtils.timeFromString(config.getString("update"));
	...
	private void build(Player p) {
		gui = new GUI(PlaceholderAPISupport.replace(title, p, true), layout.lines.size() * 9);
