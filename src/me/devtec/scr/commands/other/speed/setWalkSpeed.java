 ...
 * 25.12. 2021
 **/
public class setWalkSpeed extends CommandHolder {
	
	public setWalkSpeed(String command) {
		super(command, 0);
	}
	...
	private void apply(CommandSender s, Player player, double level, boolean silent) {
		...
		player.setWalkSpeed((float)level/10);
		if(!silent) {
			if(player==s) {
				Loader.send(s, "setWalkSpeed.self", PlaceholderBuilder.make(s, "sender").add("value", Formatters.formatDouble(level)));
			}else {
				Loader.send(s, "setWalkSpeed.other.sender", PlaceholderBuilder.make(s, "sender").player(player, "target").add("value", Formatters.formatDouble(level)));
				Loader.send(player, "setWalkSpeed.other.target", PlaceholderBuilder.make(s, "sender").player(player, "target").add("value", Formatters.formatDouble(level)));
			}
		}
	}
