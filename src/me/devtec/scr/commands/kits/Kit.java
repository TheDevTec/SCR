...
public class Kit implements ScrCommand {
	...
	public void kit(CommandSender sender, CommandSender target, String kit_name, boolean permission,
			boolean cooldown, boolean economy, boolean silent) {
		...
		if(!permission || sender.hasPermission(kit.permission())) { // PERMISSION
			...
			if(!cooldown || u.cooldownExpired("kits."+kit.getName(), kit.getCooldownTime()) ) { 
				...
				if(kit.config.exists("commands"))
					for(String cmd : kit.config.getStringList("commands"))
						Sudo.sudoConsole(SudoType.COMMAND, 
								PlaceholderAPISupport.replace(cmd, target, true, Placeholders.c().addPlayer("player", target)));
				//MESSAGE
				...
				msgSec(sender, "give.sender", Placeholders.c()
						.addPlayer("target", target)
						.add("cooldown", StringUtils.timeFromString(kit.getCooldownTime()))
						.add("kit", kit.displayName()).add("kit_name", kit.getName())
						.add("cost", ((Economy) Loader.economy).format(
				...
				msgSec(target, "cooldown", Placeholders.c()
						.add("time", StringUtils.timeToString(u.expires("kits."+kit.getName(), kit.getCooldownTime())))
						.add("kit", kit.displayName()).add("kit_name", kit.getName()));
			return;
			}
