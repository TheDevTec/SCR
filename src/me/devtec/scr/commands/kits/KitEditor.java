		...
		CommandStructure.create(CommandSender.class, PERMS_CHECKER, (s, structure, args) -> { // cmd
			...
			msgSec(s, "list", Placeholders.c().add("kits", loaded));
				...
				.argument("create", (s, structure, args) -> { // cmd create
					help(s, "create");
				}).permission(permission("create")).argument(null, (s, structure, args) -> { // cmd create [kit]
				...
				.argument("delete", (s, structure, args) -> { // cmd delete
					help(s, "delete");
				}).permission(permission("delete")).argument(null, (s, structure, args) -> { // cmd delete [kit]
					if (KitUtils.kitExist(args[1])) { // then create
				...
				.argument("item", (s, structure, args) -> { // cmd item
					help(s, "item");
				}).permission(permission("item")).fallback((s, structure, args) -> { // cmd item [kit]
					msgSec(s, "notFound", Placeholders.c().add("kit", args[1]).add("kit_name", args[1]));
				}).callableArgument((s, structure, args) -> KitUtils.getKitsFor(s), (s, structure, args) -> { // cmd item [kit]
					help(s, "item");
					...
					if (kit != null) {
						...
						if (kit.config.exists("items.add." + id)) { // if item already exist in kit
							msgSec(s, "item.exist", Placeholders.c().add("kit", kit.displayName()).add("kit_name", kit.getName()).add("item", id));
							return;
						}
						Player p = (Player) s;
						...
						if (hand != Material.AIR) {
							...
							msgSec(s, "item.add", Placeholders.c().add("kit", kit.displayName()).add("kit_name", kit.getName()).add("item", id));
						} else
				...
				.argument("set", (s, structure, args) -> { // cmd item [kit] set [slot]
					...
					if (kit != null) {
						...
						if (hand != Material.AIR) {
							...
							msgSec(s, "item.set", Placeholders.c().add("kit", kit.displayName()).add("kit_name", kit.getName()).add("slot", slot));
						} else
							msg(s, "missing.handEmpty");
					}
