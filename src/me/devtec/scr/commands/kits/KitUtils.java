	...
	public static void loadKits() { // load & reload
		...
		if (file.exists() && file.isDirectory())
			for (File f : file.listFiles()) {
				...
				} else if (c.getBoolean("enabled")) { // Reloading kits config
	...
	public static class Kit {
		...
		public String displayName() {
			if (config.exists("displayName"))
				return ColorUtils.colorize(config.getString("displayName"));
			return config.getString("name");
		}
		...
		public void giveItems(CommandSender sender) {
			...
			if (config.exists("items.set"))
				for (String sslot : config.getKeys("items.set")) {
					int slot = ParseUtils.getInt(sslot);
		...
		public List<String> getItems() {
			List<String> list = new ArrayList<>(config.getKeys("items.add"));

			list.addAll(config.getKeys("items.set"));
