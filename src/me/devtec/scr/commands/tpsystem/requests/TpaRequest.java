...
public class TpaRequest implements TeleportRequest {
	...
	public void cancel() {
		if (isFinished())
			return;
		finish();
		MessageUtils.message(requester.getPlayer(), "teleportreq.tpa.cancel.sender", Placeholders.c().addPlayer("player", target.getPlayer()).addPlayer("target", requester.getPlayer()));
	...
	public void timeout() {
		...
		finish();
		if (target.isOnline())
			MessageUtils.message(requester.getPlayer(), "teleportreq.tpa.expired.online", Placeholders.c().addPlayer("player", target.getPlayer()));
		else
			MessageUtils.message(requester.getPlayer(), "teleportreq.tpa.expired.offline", Placeholders.c().add("player", target.getName()));
		removeRequest();
	}
