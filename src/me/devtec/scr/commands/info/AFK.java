...
public class AFK implements ScrCommand {
	...
	public void init(List<String> cmds) {
		...
		CommandStructure.create(CommandSender.class, PERMS_CHECKER, (s, structure, args) -> { // afk
			if (players.containsKey(s.getName())) { // turn off
				MessageUtils.message(s, configSection() + ".end", Placeholders.c().addPlayer("player", s), false, API.getOnlinePlayersThatcanSee(s, true).toArray(new CommandSender[0]));
				players.remove(s.getName());
			} else { // turn on
				players.put(s.getName(), System.currentTimeMillis() / 1000);
				MessageUtils.message(s, configSection() + ".start", Placeholders.c().addPlayer("player", s), false, API.getOnlinePlayersThatcanSee(s, true).toArray(new CommandSender[0]));
			}
		}).cooldownDetection((s, structure, args) -> inCooldown(s)).permission(permission("cmd")).argument("-s", (s, structure, args) -> { // afk -s
			if (players.containsKey(s.getName()))
				players.remove(s.getName());
			else
				players.put(s.getName(), System.currentTimeMillis() / 1000);
		}).priority(1).parent() // afk
				.argument(null, (s, structure, args) -> { // afk [reason]
					if (players.containsKey(s.getName())) { // turn off
						...
						players.put(s.getName(), System.currentTimeMillis() / 1000);
						MessageUtils.message(s, configSection() + ".start_reson", Placeholders.c().addPlayer("player", s).add("reason", StringUtils.buildString(args)), false,
								API.getOnlinePlayersThatcanSee(s, true).toArray(new CommandSender[0]));
					}
				}).priority(2).first() // afk
				.fallback((s, structure, args) -> {
					...
					for (Player p : playerSelectors(s, args[0]))
						if (players.containsKey(p.getName())) { // turn off
							MessageUtils.message(p, configSection() + ".end", Placeholders.c().addPlayer("player", p), false, API.getOnlinePlayersThatcanSee(p, true).toArray(new CommandSender[0]));
				...
				}).permission(permission("other")).argument("-s", (s, structure, args) -> { // afk [player] -s
					for (Player p : playerSelectors(s, args[0]))
				...
				.argument(null, (s, structure, args) -> {
					for (Player p : playerSelectors(s, args[0]))
						if (players.containsKey(p.getName())) { // turn off
							...
							players.put(p.getName(), System.currentTimeMillis() / 1000);
							MessageUtils.message(p, configSection() + ".start_reson", Placeholders.c().addPlayer("player", p).add("reason", StringUtils.buildString(1, args)), false,
									API.getOnlinePlayersThatcanSee(p, true).toArray(new CommandSender[0]));
						}
	...
	public static void afk(CommandSender player, boolean enable) {
		if (enable) { // turn on
			...
			MessageUtils.message(player, "afk.start", Placeholders.c().addPlayer("player", player), false, API.getOnlinePlayersThatcanSee(player, true).toArray(new CommandSender[0]));
		} else { // turn off
			MessageUtils.message(player, "afk.end", Placeholders.c().addPlayer("player", player), false, API.getOnlinePlayersThatcanSee(player, true).toArray(new CommandSender[0]));
