		...
		CommandStructure.create(CommandSender.class, PERMS_CHECKER, (s, structure, args) -> {
			...
			for (Player staff_player : staff) {
				...
				staff_b.append( PlaceholderAPISupport.replace(
						Loader.translations.getString(configSection() + ".format.staff"), staff_player, true));
			}
			...
			for (Player player_p : players) {
				...
				player_b.append( PlaceholderAPISupport.replace(
						Loader.translations.getString(configSection() + ".format.player"), player_p, true));
			}
			msgSec(s, "message", Placeholders.c().add("staff", staff_b.isEmpty()?"-":staff_b)
					.add("players", player_b.isEmpty()?"-":player_b)
