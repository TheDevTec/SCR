...
public class Repair implements ScrCommand {
	...
	public void init(List<String> cmds) {
		...
		}).cooldownDetection((s, structure, args) -> inCooldown(s)).permission(permission("cmd")).argument("-s", (s, structure, args) -> { // repair -s
			if (!(s instanceof Player)) {
				help(s, "usage");
				return;
			}
			...
			if (hand != Material.AIR) {
				...
				.argument("all", (s, structure, args) -> { // repair ALL
					...
					for (ItemStack t : items) {
						if (t != null && t.getType() != Material.AIR && isTool(t.getType().name())) {
					...
					msgSec(s, "all.you");
				}, "ALL").permission(permission("all")).argument("-s", (s, structure, args) -> { // repair ALL -s
					if (!(s instanceof Player)) {
						help(s, "usage");
						return;
					}
				...
				}).permission(permission("other")).argument("-s", (s, structure, args) -> { // repair [player] -s
					for (Player p : playerSelectors(s, args[0])) {
						Material hand = p.getItemInHand().getType();
						if (hand != Material.AIR) {
							if (isTool(p.getItemInHand().getType().name())) {
				...
				}, "ALL").permission(permission("all")).build().register(cmds.remove(0), cmds.toArray(new String[0]));
	}
	...
	private boolean isTool(String name) {
		return name.endsWith("_PICKAXE") || name.endsWith("_AXE") || name.endsWith("_SPADE") || name.endsWith("_SHOVEL") || name.endsWith("_HOE") || name.endsWith("_HELMET") || name.endsWith("_BOOTS")
				|| name.endsWith("_ON_A_STICK") || name.endsWith("_LEGGINGS") || name.endsWith("_CHESTPLATE") || name.endsWith("_SWORD") || name.equals("BOW") || name.equals("SHEARS")
				|| name.equals("FLINT_AND_STEEL") || name.equals("TRIDENT") || name.equals("ELYTRA") || name.equals("CROSSBOW") || name.equals("SHIELD") || name.equals("FISHING_ROD");
	}
}
