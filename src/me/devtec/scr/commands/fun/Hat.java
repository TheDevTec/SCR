...
public class Hat implements ScrCommand {
	...
	public void init(List<String> cmds) {
		...
		.permission(permission("cmd"))
			.argument("-s", (s, structure, args) -> { // hat -s
			...
			.selector(Selector.ENTITY_SELECTOR, (s, structure, args) -> { // hat [player]
				for(Player target : playerSelectors(s, args[0])) {
					if (s.getItemInHand().getType() != Material.AIR) {
						...
						if (target.getInventory().getHelmet() != null && check) {
							target.getInventory().addItem(target.getInventory().getHelmet());
						}else { // FULL INV
							msgSec(s, "fullInv", Placeholders.c()
									.add("item", s.getInventory().getHelmet().getType().name())
						...
						msgSec(s, "equipped.other.sender", Placeholders.c()
								.add("item", s.getInventory().getHelmet().getType().name())
								.addPlayer("target", target));
						msgSec(target, "equipped.other.target", Placeholders.c()
								.add("item", s.getInventory().getHelmet().getType().name())
								.addPlayer("player", s));
					} else
						msg(s, "missing.handEmpty");
				...
				.argument("-s", (s, structure, args) -> { // hat [player] -s
					for(Player target : playerSelectors(s, args[0])) {
						if (s.getItemInHand().getType() != Material.AIR) {
							//INV free slot checker
							Inventory inv = target.getInventory();
