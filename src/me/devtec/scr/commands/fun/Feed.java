	...
	public void init(List<String> cmds) {
		CommandStructure.create(CommandSender.class, PERMS_CHECKER, (s, structure, args) -> { // feed
			if (s instanceof Player) {
				...
				help(s, "usage");
		}).cooldownDetection((s, structure, args) -> inCooldown(s)).permission(permission("cmd")).argument("-s", (s, structure, args) -> { // feed -s
			if (s instanceof Player) {
				Player p = (Player) s;
				p.setFoodLevel(20);
			} else
				...
				.fallback((s, structure, args) -> { // feed {level}
					...
					if (s instanceof Player) {
						Player p = (Player) s;
						int level = ParseUtils.getInt(args[0]);
						if (level < 0)
							level = 0;
						if (level > 20)
							level = 20;
					...
					for (int i = 0; i <= 20; i++)
						list.add("" + i);
					return list;
				}).permission(permission("food_level")).argument("-s", (s, structure, args) -> { // feed {level} -s
					if (s instanceof Player) {
						Player p = (Player) s;
						int level = ParseUtils.getInt(args[0]);
						if (level < 0)
				...
				}).permission(permission("other")).argument("-s", (s, structure, args) -> { // feed [player] -s
					for (Player p : playerSelectors(s, args[0]))
						p.setFoodLevel(20);
				}).parent() // feed [player]
				.fallback((s, structure, args) -> { // feed [player] {level}
					...
					for (Player p : playerSelectors(s, args[0])) {
						int level = ParseUtils.getInt(args[1]);
						if (level < 0)
							level = 0;
						if (level > 20)
							level = 20;
					...
					for (int i = 0; i <= 20; i++)
						list.add("" + i);
					return list;
				}).permission(permission("food_level")).argument("-s", (s, structure, args) -> { // feed [player] {level} -s
					for (Player p : playerSelectors(s, args[0])) {
						int level = ParseUtils.getInt(args[1]);
						if (level < 0)
							level = 0;
