	...
	public void init(List<String> cmds) {
		...
		.argument("list", (s, structure, args) -> {   // /cmd [list]
			...
			for(String nickname: Loader.data.getKeys("nicknames"))
				msg(s, "nickname.list.format", Placeholders.c().replace("name", Loader.data.getString("nicknames."+nickname))
						.replace("nickname", nickname));
		}).priority(1).permission(permission("list"))
		.parent() // cmd <-
		.argument(null, (s, structure, args) -> {
			User u = API.getUser(s);
			if(!u.isConsole()) {
				msg(s, "nickname.change.target", Placeholders.c().addPlayer("player", u.player).replace("nickname", args[0]) );
				u.setNickname(args[0]);
			} else
				help(s, "admin_usage");
		}).permission(permission("cmd"))
			.argument("-s", (s, structure, args) -> {  // /cmd [nickname] -s
				User u = API.getUser(s);
				if(!u.isConsole()) {
					u.setNickname(args[0]);
				} else
			...
			.selector(Selector.PLAYER, (s, structure, args) -> { // /cmd [nickname] [player]
				...
				msg(s, "nickname.change.sender", Placeholders.c().addPlayer("target", u.player)/*.replace("target", u.getName())*/.replace("nickname", args[0]) );
				...
				.argument("-s", (s, structure, args) -> {  // /cmd [nickname] [player] -s
					User u = API.getUser(args[1]);
