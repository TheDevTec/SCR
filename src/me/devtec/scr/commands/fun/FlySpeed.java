...
public class FlySpeed  implements ScrCommand {
	...
	public void init(List<String> cmds) {
		...
		.permission(permission("cmd")) // perm
			...
			.selector(Selector.NUMBER, (s, structure, args) -> {  // fspeed [level]
				...
				double modifier = StringUtils.getDouble(args[0]);
				if (modifier > 10.0)
					modifier = 10.0;
				if (modifier < -10.0)
					modifier = -10.0;
				((Player) s).setFlySpeed((float) modifier / 10);
		...
		.parent()  // fspeed
			...
			.selector(Selector.ENTITY_SELECTOR, (s, structure, args) -> { // fspeed [player]
				...
				.fallback((s, structure, args) -> { // fspeed [player] [level]
				msg(s, "missing.number", Placeholders.c().replace("number", args[0]));
				})
				.selector(Selector.NUMBER, (s, structure, args) -> {  // fspeed [player] [level]
					for(Player p : playerSelectors(s, args[0])) {
						double modifier = StringUtils.getDouble(args[1]);
						if (modifier > 10.0)
							modifier = 10.0;
						if (modifier < -10.0)
							modifier = -10.0;
						p.setFlySpeed((float) modifier / 10);
