...
public class Utils {
	...
	public static boolean cooldownExpired(User user, String command) { //Check if cooldown on command expired
		...
		for(PermissionAttachmentInfo permission : user.player.getEffectivePermissions()) {
			if(permission.getPermission().toLowerCase().startsWith("scr.bypass.cooldowns."+command.toLowerCase()))
				permcooldown = ""+StringUtils.timeFromString( permission.getPermission().toLowerCase().replace(
						"scr.bypass.cooldowns."+command.toLowerCase()+".", "") );
		}
		...
		if(Loader.commands.exists(command+".cooldown")) {
			if(!user.cooldownExpired("cooldowns.commands."+command,
				...
				if(Loader.commands.exists(command+".permission.cd-bypass") &&
						user.isAutorized(Loader.commands.getString(command+".permission.cd-bypass")))
					return true;
				return false;
			}
	...
	public static long expires(User user, String command) { //How long before it expires (return seconds)
		...
		for(PermissionAttachmentInfo permission : user.player.getEffectivePermissions()) {
			if(permission.getPermission().toLowerCase().startsWith("scr.bypass.cooldowns."+command.toLowerCase()))
				permcooldown = ""+StringUtils.timeFromString( permission.getPermission().toLowerCase()
						.replace("scr.bypass.cooldowns."+command.toLowerCase()+".", "") );
		}
		if(!permcooldown.equalsIgnoreCase("0")) {
