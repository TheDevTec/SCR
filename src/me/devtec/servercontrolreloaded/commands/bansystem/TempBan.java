	...
	public boolean onCommand(CommandSender s, Command arg1, String arg2, String[] args) {
		if (Loader.has(s, "TempBan", "BanSystem")) {
			...
			if (args.length == 1) {
				...
				TheAPI.getPunishmentAPI().ban(args[0], StringUtils.getTimeFromString(Loader.config.getString("BanSystem.TempBan.Time")), Loader.config.getString("BanSystem.TempBan.Text").replace("%reason%",
								Loader.config.getString("BanSystem.TempBan.Reason")));
			...
			if (args.length == 2) {
				...
				TheAPI.getPunishmentAPI().ban(args[0], StringUtils.getTimeFromString(args[1]), Loader.config.getString("BanSystem.TempBan.Text").replace("%reason%",
								Loader.config.getString("BanSystem.TempBan.Reason")));
				Loader.sendMessages(s, "BanSystem.TempBan.Sender", Placeholder.c().replace("%operator%", s.getName())
			...
			if(msg.endsWith("-s")||msg.endsWith("- s")) {
				...
				TheAPI.getPunishmentAPI().ban(args[0], StringUtils.timeFromString(args[1]), Loader.config.getString("BanSystem.TempBan.Text").replace("%reason%",msg));
				Loader.sendMessages(s, "BanSystem.TempBan.Sender", Placeholder.c().replace("%operator%", s.getName())
