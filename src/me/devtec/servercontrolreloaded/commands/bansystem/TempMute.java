...
public class TempMute implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender s, Command arg1, String arg2, String[] args) {
		if (Loader.has(s, "TempMute", "BanSystem")) {
			if(!CommandsManager.canUse("BanSystem.TempMute", s)) {
				Loader.sendMessages(s, "Cooldowns.Commands", Placeholder.c().add("%time%", StringUtils.timeToString(CommandsManager.expire("BanSystem.TempMute", s))));
				return true;
			}
			if (args.length == 0) {
			...
			if (args.length == 1) {
				...
				TheAPI.getPunishmentAPI().mute(args[0], StringUtils.getTimeFromString(Loader.config.getString("BanSystem.TempMute.Time")), Loader.config.getString("BanSystem.TempMute.Text").replace("%reason%",
								Loader.config.getString("BanSystem.TempMute.Reason")));
			...
			if (args.length == 2) {
				...
				TheAPI.getPunishmentAPI().mute(args[0], StringUtils.getTimeFromString(args[1]), Loader.config.getString("BanSystem.TempMute.Text").replace("%reason%",
								Loader.config.getString("BanSystem.TempMute.Reason")));
			...
			if(msg.endsWith("-s")||msg.endsWith("- s")) {
				...
				TheAPI.getPunishmentAPI().mute(args[0], StringUtils.timeFromString(args[1]), Loader.config.getString("BanSystem.TempMute.Text").replace("%reason%",msg));
				Loader.sendMessages(s, "BanSystem.TempMute.Sender", Placeholder.c().replace("%operator%", s.getName())
