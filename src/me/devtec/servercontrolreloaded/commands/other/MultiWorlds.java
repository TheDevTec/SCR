...
public class MultiWorlds implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
		...
		if(args[0].equalsIgnoreCase("set")||args[0].equalsIgnoreCase("edit")){
			...
			if(!existsFlag(args[2])) {
				Loader.sendMessages(s,"Missing.Flag", Loader.Placeholder.c().add("%world%",args[1]).add("%flag%",args[2]));
				return true;
			}
	...
	static {
		...
		flags.put("GAMEMODE", TheAPI.isNewerThan(7)?Arrays.asList("SURVIVAL","CREATIVE","SPECTATOR","ADVENTURE"):Arrays.asList("SURVIVAL","CREATIVE","ADVENTURE"));
	}
