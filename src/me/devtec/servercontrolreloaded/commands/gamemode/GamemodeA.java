...
public class GamemodeA implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender s, Command arg1, String arg2, String[] args) {
		if (Loader.has(s, "GameModeAdventure", "GameMode")) {
			if(!CommandsManager.canUse("GameMode.GameModeAdventure", s)) {
				Loader.sendMessages(s, "Cooldowns.Commands", Placeholder.c().add("%time%", StringUtils.timeToString(CommandsManager.expire("GameMode.GameModeAdventure", s))));
				return true;
			}
			if (args.length == 0) {
	...
	public List<String> onTabComplete(CommandSender s, Command arg1,
		...
		if (Loader.has(s, "GameModeAdventure", "GameMode") && args.length==1)
