...
public class Spawn implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
		if (Loader.has(s, "Spawn", "Warps")) {
			if(!CommandsManager.canUse("Warps.Spawn", s)) {
				Loader.sendMessages(s, "Cooldowns.Commands", Placeholder.c().add("%time%", StringUtils.timeToString(CommandsManager.expire("Warps.Spawn", s))));
				return true;
			}
			if (args.length == 0) {
