...
public class CloseInventory implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender s, Command arg1, String arg2, String[] args) {
		if (Loader.has(s, "CloseInventory", "Inventory")) {
			if(!CommandsManager.canUse("Inventory.CloseInventory", s)) {
				Loader.sendMessages(s, "Cooldowns.Commands", Placeholder.c().add("%time%", StringUtils.timeToString(CommandsManager.expire("Inventory.CloseInventory", s))));
				return true;
			}
			if (args.length == 0) {
