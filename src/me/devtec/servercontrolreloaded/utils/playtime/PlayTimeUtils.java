...
public class PlayTimeUtils {
	...
	public enum PlayTimeType {
		GLOBAL, GAMEMODE, WORLD, WORLDGAMEMODE
	}
	...
	public static long playtime(Player s) {
		...
		if (!Loader.config.getBoolean("Options.PlayTime.UseCustomPlayTime"))
			...
			return new Data("plugins/TheAPI/User/" + s.getUniqueId() + ".yml").getInt("Statistics.PlayTime");
	}
	...
	public static long playtime(Player s, String path) {
		...
		if (!Loader.config.getBoolean("Options.PlayTime.UseCustomPlayTime"))
			...
			return new Data("plugins/TheAPI/User/" + s.getUniqueId() + ".yml").getInt("Statistics." + path);
	}
	...
	public static void req() {
		if (old - System.currentTimeMillis() / 1000 + 20 * 60 * 5 <= 0) {
			PlayTimeUtils.playtop.clear();
			for (UUID sa : TheAPI.getUsers()) {
				String n = LoaderClass.cache.lookupNameById(sa);
				if (n != null) {
					if (PlayTimeUtils.playtop.containsKey(n))
						continue;
					int time = new Data("plugins/TheAPI/User/" + sa + ".yml").getInt("Statistics.PlayTime");
					if (time > 0)
	...
	public static void loadRewards() {
		for (String reward : Loader.rewards.getKeys("PlayTime")) {
			...
			TheAPI.getConsole().sendMessage("&4ERROR: Playtime reward &c" + reward
					+ " &6is not valid! Fix this playtime reward in Rewards.yml");
		}
		if (!playrewards.isEmpty()) {
			TheAPI.msg(setting.prefix + " &8*********************************************", TheAPI.getConsole());
			TheAPI.msg(setting.prefix + " &fLoaded &6" + playrewards.size() + " &fPlayTime Rewards",
					TheAPI.getConsole());
		}
	}
