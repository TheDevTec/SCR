		...
		public boolean PacketPlayOut(String player, Object packet, Object channel) {
			if(packet.getClass()==p) {
				...
				if(TheAPI.isOlderThan(8)) {
					Ref.set(packet, "username", TabList.getTabListName(TheAPI.getPlayer(player)));
				}else {
					for(Object info : (List<?>)Ref.get(packet, "b"))
					Ref.set(info, "e", NMSAPI.getFixedIChatBaseComponent(TabList.getTabListName(TheAPI.getPlayer(player))));
				}
			}
			return false;
		}
