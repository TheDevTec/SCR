...
public class AFKPlus implements Listener {
	...
	public void onMove(AFKStartEvent e) {
		if (!AFKPlus.containsKey(TheAPI.getPlayer(e.getPlayer().getName())))
			AFKPlus.put(TheAPI.getPlayer(e.getPlayer().getName()), e.getPlayer());
	}

	@EventHandler(priority = EventPriority.LOWEST)
	public void onMove(AFKStopEvent e) {
		if (AFKPlus.containsKey(TheAPI.getPlayer(e.getPlayer().getName())))
			AFKPlus.remove(TheAPI.getPlayer(e.getPlayer().getName()));
	}
}
