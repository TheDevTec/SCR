...
public void CooldownChat(PlayerChatEvent e) {
	...
	if(!p.hasPermission("ServerControl.CooldownBypass.Chat") && Loader.config.getInt("Cooldown.Chat") > 0) {
...
public void CooldownCommands(PlayerCommandPreprocessEvent e) {
	Player p = e.getPlayer();
	if(!p.hasPermission("ServerControl.CooldownBypass.Commands") && Loader.config.getInt("Cooldown.Commands") > 0) {
