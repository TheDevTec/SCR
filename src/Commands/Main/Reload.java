...
public class Reload implements CommandExecutor, TabCompleter {
	void stop(CommandSender s) {
		...
		Bukkit.savePlayers();
		TheAPI.broadcastMessage("&cReload of server..");
		Bukkit.reload();
	}
	TheRunnable r;
	public boolean stop;
	...
	public boolean onCommand(CommandSender s, Command arg1, String arg2, String[] args) {
		if(API.hasPerm(s, "ServerControl.Restart")) {
			int time = Loader.config.getInt("Options.WarningSystem.Reload.PauseTime");
			if(args.length==1) {
				if(args[0].equalsIgnoreCase("cancel")) {
					r.cancel();
					stop=false;
					TheAPI.broadcastMessage("&eServer reload has been &acanceled");
					return true;
				}else
