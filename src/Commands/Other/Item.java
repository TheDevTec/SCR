	...
	public boolean onCommand(CommandSender s, Command arg1, String arg2, String[] args) {
		if (API.hasPerm(s, "ServerControl.Item")) {
			if (s instanceof Player) {
				if (args.length == 0) {
					Loader.Help(s, "/Item Name <name>", "Item.SetName");
					Loader.Help(s, "/Item Lore", "Item.SetLore.Add");
					Loader.Help(s, "/Item Lore Add <lore>", "Item.SetLore.Add");
					Loader.Help(s, "/Item Lore Remove <line>", "Item.SetLore.Remove");
					Loader.Help(s, "/Item Flag <flag> <true/false>", "Item.HideEnchants");
					return true;
				}
				Player p = (Player) s;
				...
				if (item.getType() != Material.AIR) {
					...
					if (!is(args, "setname") || !is(args, "setlore") || !is(args, "HideEnchants")
							|| !is(args, "unbreakable")) {
						Loader.Help(s, "/Item SetName <name>", "Item.SetName");
		...
		if (s.hasPermission("ServerControl.Item")) {
			if (args.length == 1) {
				c.addAll(StringUtil.copyPartialMatches(args[0],
						Arrays.asList("SetName", "SetLore", "Unbreakable", "HideEnchants"), new ArrayList<>()));
			}
			if (args.length == 2) {
				if (args[0].equalsIgnoreCase("setname"))
				...
				if (args[0].equalsIgnoreCase("SetLore"))
					c.addAll(StringUtil.copyPartialMatches(args[1], Arrays.asList("Add", "Remove", "Lines"),
							new ArrayList<>()));
			}
			if (args.length == 3) {
				...
				if (args[1].equalsIgnoreCase("Remove")) {
					...
					if (s instanceof Player && ((Player) s).getItemInHand() != null
							&& ((Player) s).getItemInHand().hasItemMeta()
							&& ((Player) s).getItemInHand().getItemMeta().hasLore())
						for (@SuppressWarnings("unused")
						String d : ((Player) s).getItemInHand().getItemMeta().getLore()) {
							l.add(count + "");
							count++;
						}
					if (!l.isEmpty())
