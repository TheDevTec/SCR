...
public class Spawner implements CommandExecutor, TabCompleter {
	...
	public static List<String> list() {
		...
		String[] d = {"FISHING_HOOK","DROPPED_ITEM","LEASH_HITCH","LIGHTNING","PLAYER","MINECART_MOB_SPAWNER","UKNOWN","FIREWORK","PRIMED_TNT", "AREA_EFFECT_CLOUD","ENDER_SIGNAL",
				"UNKNOWN"};
			...
			if (s instanceof Player) {
				...
				if(TheAPI.isNewVersion()) {
					...
					Loader.msg(Loader.s("Spawner.AmountSet")
							...
							.replace("%y%", String.valueOf(b.getY())), s);
					return true;
					}
				...
				if (args[0].equalsIgnoreCase("setMob")) {
					...
					if (b.getType().name() == "SPAWNER") {
						...
						if (type != null) {
							...
							ss.update();
							Loader.msg(Loader.s("Spawner.Set")
					...
					Loader.msg(Loader.s("Spawner.BlockIsNotSpawner").replace("%block%", b.getType().name())
							.replace("%world%", b.getWorld().getName())
							.replace("%x%", String.valueOf(b.getX()))
							.replace("%z%", String.valueOf(b.getZ()))
							.replace("%y%", String.valueOf(b.getY())), s);
