...
public class Mail implements CommandExecutor, TabCompleter {
	...
	public List<String> onTabComplete(CommandSender s, Command arg1, String arg2, String[] args) {
		...
		if (s.hasPermission("ServerControl.Mail.Read")) { // pokud m» permise ...
			if (args.length == 1) {
				c.addAll(StringUtil.copyPartialMatches(args[0], Arrays.asList("Read", "Clear"), new ArrayList<>())); // p»id»
																														// tam
																														// Read
																														// a
																														// Clear
		...
		if (s.hasPermission("ServerControl.Mail.Send")) {
			if (args.length == 1) {
				c.addAll(StringUtil.copyPartialMatches(args[0], Arrays.asList("Send"), new ArrayList<>())); // P»id»
																											// Send
			}
			if (args[0].equalsIgnoreCase("Send") && args.length == 2)
				return null; // returne online hr»»e
			if (args.length >= 3)
